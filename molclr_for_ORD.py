...
def _save_config_file(model_checkpoints_folder):
    if not os.path.exists(model_checkpoints_folder):
        ...
        shutil.copy('./config_for_ORD.yaml', os.path.join(model_checkpoints_folder, 'config_for_ORD.yaml'))
...
class MolCLR(object):
    def __init__(self, dataset, config):
        ...
        log_dir = os.path.join('ckpt_for_ORD', dir_name)
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            from models.ginet_molclr_for_ORD import GINet
        ...
        model_checkpoints_folder = os.path.join(self.writer.log_dir, 'checkpoints_for_ORD')
    ...
    def _load_pre_trained_weights(self, model):
        try:
            checkpoints_folder = os.path.join('./ckpt_for_ORD', self.config['load_model'], 'checkpoints_for_ORD')
...
def main():
    config = yaml.load(open("config_for_ORD.yaml", "r"), Loader=yaml.FullLoader)
    print(config)

    if config['aug'] == 'node':
        from dataset.dataset_for_ORD import MoleculeDatasetWrapper
